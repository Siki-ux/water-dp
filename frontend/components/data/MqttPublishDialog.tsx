...
export default function MqttPublishDialog({
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-background/80 backdrop-blur-md p-4">
            <div className="bg-card border border-border rounded-xl w-full max-w-lg shadow-2xl flex flex-col">
                <div className="flex items-center justify-between p-4 border-b border-border">
                    <h3 className="text-lg font-semibold text-[var(--foreground)]">Publish MQTT Message</h3>
                ...
                <div className="p-6 space-y-4">
                    <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 text-sm text-blue-300">
                        Publishing to sensor: <span className="font-semibold text-[var(--foreground)]">{sensorName}</span>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-[var(--foreground)]/70 mb-1">Topic Suffix</label>
                        <input
                            type="text"
                            value={topicSuffix}
                        ...
                        />
                        <p className="text-xs text-[var(--foreground)]/40 mt-1">Appended to base topic (e.g. <code>.../{topicSuffix}</code>)</p>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-[var(--foreground)]/70 mb-1">JSON Payload</label>
                        <textarea
                            value={jsonPayload}
                            onChange={(e) => setJsonPayload(e.target.value)}
                            className="w-full h-40 bg-muted/50 border border-border rounded-lg px-3 py-2 text-[var(--foreground)] font-mono text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 resize-none"
                            placeholder="{ ... }"
                        />
                    </div>
                ...
                <div className="p-4 border-t border-border flex justify-end gap-2">
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-[var(--foreground)] rounded-lg text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
