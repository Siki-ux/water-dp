...
@patch("app.services.dashboard_service.ProjectService._check_access")
def test_get_dashboard_private_authenticated(
    mock_check_access, mock_db_session, mock_user
):
    dashboard_id = uuid.uuid4()
    project_id = uuid.uuid4()
    dashboard = Dashboard(
        id=dashboard_id, is_public=False, project_id=project_id, name="Private"
    )
    mock_db_session.query.return_value.filter.return_value.first.return_value = (
        dashboard
    )
    ...
    result = DashboardService.get_dashboard(
        mock_db_session, dashboard_id, user=mock_user
    )
    assert result.name == "Private"
    mock_check_access.assert_called_with(
        mock_db_session, project_id, mock_user, required_role="viewer"
    )
...
def test_update_dashboard(mock_check_access, mock_db_session, mock_user):
    ...
    DashboardService.update_dashboard(
        mock_db_session, dashboard_id, dashboard_in, mock_user
    )
