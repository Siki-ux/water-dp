...
def seed_keycloak_users_and_groups():
    ...
    for attempt in range(30):
        try:
            ...
            if resp.status_code == 200:
                ...
                break
            logger.info(
                f"Keycloak not ready yet (attempt {attempt+1}/30, status {resp.status_code}), retrying in 5s..."
            )
        except Exception as e:
            logger.info(
                f"Keycloak not reachable yet (attempt {attempt+1}/30): {e}, retrying in 5s..."
            )
        time.sleep(5)

    if not token:
        logger.error(
            "Keycloak admin auth failed after 30 attempts. Skipping Keycloak seed."
        )
        return
    ...
    if cu:
        for role in ("viewer", "editor", "admin"):
            r = requests.post(
                f"{kc_url}/admin/realms/{realm}/clients/{cu}/roles",
                headers=headers,
                json={"name": role},
            )
    ...
    for group_name, subgroup_members in SEED_GROUPS:
        ...
        for sg_name, role_name in SUBGROUP_ROLES.items():
            ...
            if not sg_id:
                logger.error(
                    f"Could not ensure subgroup '{sg_name}' under '{group_name}'"
                )
                continue
            ...
            for username in subgroup_members.get(sg_name, []):
                ...
                if user:
                    ...
                    if r.status_code == 204:
                        ...
                        logger.warning(
                            f"  Could not add '{username}' to {group_name}/{sg_name}: {r.text}"
                        )

    logger.info("Keycloak user/group seeding complete.")
    logger.info(
        "  Users (password = username): alice, bob, carol, dave, eva, frank, grace"
    )
