...
class QcStream(BaseModel):
    ...
    arg_name: str = Field(
        ..., description="SaQC function argument name for this stream"
    )
...
class QAQCTestBase(BaseModel):
    ...
    args: Optional[Dict[str, Any]] = Field(
        None, description="Function keyword arguments"
    )
...
class QAQCConfigResponse(QAQCConfigBase):
    id: int
    tsm_project_id: Optional[int] = Field(
        None, description="TSM ConfigDB project ID (null for per-sensor configs)"
    )
