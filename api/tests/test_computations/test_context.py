...
class TestComputationContextInit:
    ...
    def test_raises_when_no_project_found(self):
        """If script not found, RuntimeError is raised instead of silently continuing."""
        db = MagicMock()
        ...
        with (
            ...
            with pytest.raises(RuntimeError, match="could not resolve project schema"):
                ComputationContext(db=db, job_id="j", script_id=uuid.uuid4(), params={})

    def test_no_project_name_when_script_missing(self):
        """If script not found, RuntimeError is raised."""
        db = MagicMock()
        db.query.return_value.filter.return_value.first.return_value = None

        with patch(FROST_PATCH), patch(SETTINGS_PATCH) as s:
            s.frost_url = "http://frost"
            s.frost_version = "v1.1"
            s.frost_server = "frost"
    ...
    def test_no_project_name_when_project_has_no_schema(self):
        """If project.schema_name is falsy, RuntimeError is raised."""
        db = MagicMock()
        mock_script = MagicMock()
        mock_script.project_id = uuid.uuid4()
        mock_project = MagicMock()
